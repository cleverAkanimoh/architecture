  ...
  return (
    <header className={`${navFixedState ? 'fixed' : ''} header-style`}>
      ...
      <nav className='nav-style'>
        ...
        <div className={`${isMenuPressed ? 'h-full' : 'h-0'} w-full transition-custom md:h-full `}>
          <div className={`${isMenuPressed ? 'translate-y-[0%]' : 'translate-y-[-100%] md:translate-y-[0%]'} nav-link-comp-container`}>
            <NavLinkComp />
          </div>
        </div>
      </nav>
